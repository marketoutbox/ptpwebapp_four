  ...
  return (
    <div className="space-y-6">
      <div className="text-center mb-8">
        <h1 className="text-3xl font-bold text-blue-50 mb-2">Pair Trading Backtest</h1>
        <p className="text-blue-200 opacity-80">Dynamic Spread Model</p>
      </div>
      ...
      <Card title="Backtest Parameters">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs text-blue-300 opacity-80 mb-1">From:</label>
                <Input type="date" value={fromDate} onChange={(e) => setFromDate(e.target.value)} />
              </div>
              <div>
                <label className="block text-xs text-blue-300 opacity-80 mb-1">To:</label>
          ...
          <div>
            <label className="block text-sm font-medium text-blue-200 mb-2">Stock Selection</label>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs text-blue-300 opacity-80 mb-1">Stock A:</label>
              ...
              <div>
                <label className="block text-xs text-blue-300 opacity-80 mb-1">Stock B:</label>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div>
            <label className="block text-sm font-medium text-blue-200 mb-2">Lookback Period (days)</label>
            <Input
              type="number"
              ...
              max="252"
            />
            <p className="mt-1 text-xs text-blue-300 opacity-80">Window size for calculating hedge ratio and z-score</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-blue-200 mb-2">Entry Z-score</label>
            <Input
              type="number"
              step="0.1"
              value={entryZ}
            ...
            />
            <p className="mt-1 text-xs text-blue-300 opacity-80">Threshold for entering a trade position</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-blue-200 mb-2">Exit Z-score</label>
            <Input
              type="number"
              step="0.1"
              value={exitZ}
            ...
            />
            <p className="mt-1 text-xs text-blue-300 opacity-80">Threshold for exiting a trade position</p>
          </div>
        </div>
          ...
          <div className="overflow-x-auto">
            <div className="max-h-64 overflow-y-auto">
              <table className="min-w-full divide-y divide-blue-800/10 data-table">
                <thead className="bg-blue-900/30 backdrop-blur-sm sticky top-0">
                  <tr>
                    <th className="text-left text-blue-200">Date</th>
                    <th className="text-left text-blue-200">{selectedPair.stockA} Close</th>
                    <th className="text-left text-blue-200">{selectedPair.stockB} Close</th>
                    <th className="text-left text-blue-200">Hedge Ratio (β)</th>
                    <th className="text-left text-blue-200">Spread (A - βB)</th>
                    <th className="text-left text-blue-200">Z-score</th>
                  </tr>
                </thead>
                <tbody className="bg-blue-900/20 backdrop-blur-sm divide-y divide-blue-800/10">
                  {backtestData.map((row, index) => (
                    <tr key={index} className={index % 2 === 0 ? "bg-blue-900/10" : "bg-blue-900/20"}>
                      <td className="whitespace-nowrap text-sm text-blue-100">{row.date}</td>
                      <td className="whitespace-nowrap text-sm text-blue-100">{row.stockAClose.toFixed(2)}</td>
                      <td className="whitespace-nowrap text-sm text-blue-100">{row.stockBClose.toFixed(2)}</td>
                      <td className="whitespace-nowrap text-sm text-blue-100">{row.hedgeRatio.toFixed(4)}</td>
                      <td className="whitespace-nowrap text-sm text-blue-100">{row.spread.toFixed(4)}</td>
                      <td
      ...
      {tradeResults.length > 0 && !isLoading && (
        <Card title="Trade Results">
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-blue-800/10 data-table">
              <thead className="bg-blue-900/30 backdrop-blur-sm">
                <tr>
                  <th className="text-left text-blue-200">Entry Date</th>
                  <th className="text-left text-blue-200">Exit Date</th>
                  <th className="text-left text-blue-200">Type</th>
                  <th className="text-left text-blue-200">Days</th>
                  <th className="text-left text-blue-200">Profit ($)</th>
                  <th className="text-left text-blue-200">Drawdown ($)</th>
                  <th className="text-left text-blue-200">Entry β</th>
                  <th className="text-left text-blue-200">Exit β</th>
                  <th className="text-left text-blue-200">β Change (%)</th>
                </tr>
              </thead>
              <tbody className="bg-blue-900/20 backdrop-blur-sm divide-y divide-blue-800/10">
                {tradeResults.map((trade, index) => (
                  <tr key={index} className={index % 2 === 0 ? "bg-blue-900/10" : "bg-blue-900/20"}>
                    <td className="whitespace-nowrap text-sm text-blue-100">{trade.entryDate}</td>
                    <td className="whitespace-nowrap text-sm text-blue-100">{trade.exitDate}</td>
                    <td
                      className={`whitespace-nowrap text-sm font-medium ${
                    ...
                    <td className="whitespace-nowrap text-sm text-blue-100">{trade.holdingPeriod}</td>
                    <td
                      className={`whitespace-nowrap text-sm font-medium ${
                    ...
                    <td className="whitespace-nowrap text-sm text-red-400">${trade.maxDrawdown}</td>
                    <td className="whitespace-nowrap text-sm text-blue-100">{trade.hedgeRatio}</td>
                    <td className="whitespace-nowrap text-sm text-blue-100">{trade.exitHedgeRatio}</td>
                    <td
          ...
          <div className="mt-6 grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="bg-blue-900/20 backdrop-blur-sm rounded-lg p-4 shadow-soft">
              <p className="text-sm text-blue-300 opacity-80">Total Trades</p>
              <p className="text-2xl font-bold text-yellow-400">{tradeResults.length}</p>
            </div>
            <div className="bg-blue-900/20 backdrop-blur-sm rounded-lg p-4 shadow-soft">
              <p className="text-sm text-blue-300 opacity-80">Profitable Trades</p>
              <p className="text-2xl font-bold text-green-400">{profitableTrades}</p>
            </div>
            <div className="bg-blue-900/20 backdrop-blur-sm rounded-lg p-4 shadow-soft">
              <p className="text-sm text-blue-300 opacity-80">Win Rate</p>
              <p className="text-2xl font-bold text-yellow-400">{winRate.toFixed(1)}%</p>
            </div>
            <div className="bg-blue-900/20 backdrop-blur-sm rounded-lg p-4 shadow-soft">
              <p className="text-sm text-blue-300 opacity-80">Avg. Profit per Trade</p>
